 ...
 */
public class APICategoriaAnimalController {
	@Autowired
	...
	@PostMapping
	public ResponseEntity<Object> saveCategoriaAnimal(@RequestBody @Valid CategoriaAnimalDTO categoriaAnimalDTO, BindingResult result) {
		...
		if (mantemCategoriaAnimal.searchByNome(categoriaAnimalDTO.getNome()).isPresent()) {
			logger.info(">>>>>> apicontroller consultaporcpf categoria ja cadastrado");
	...
	@CrossOrigin // desabilita o cors do spring security
	@GetMapping
	public ResponseEntity<List<CategoriaAnimal>> consultaTodos() {
	...
	@CrossOrigin // desabilita o cors do spring security
	@PutMapping("/{id}")
	public ResponseEntity<Object> atualiza(@PathVariable long id, @RequestBody @Valid CategoriaAnimalDTO categoriaAnimalDTO,
		...
		Optional<CategoriaAnimal> categoriaAnimal = mantemCategoriaAnimal.updates(id, categoriaAnimalDTO.retornaUmaCategoriaAnimal());
